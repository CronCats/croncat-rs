...
impl GrpcSigner {
        ...
        T: DeserializeOwned,
    {
        let out = self
            .client
            .query_client
            .query_contract(self.client.cfg.contract_address.as_ref().unwrap(), msg)
            .await?;
        Ok(out)
    }
    ...
    pub async fn execute_croncat(&self, msg: &ExecuteMsg) -> Result<TxResult, Report> {
        let res = self
            .client
            .execute_wasm(msg, self.client.cfg.contract_address.as_ref().unwrap())
            .await?;
...
impl GrpcQuerier {
    pub async fn new(cfg: ChainConfig) -> Result<Self, Report> {
        Ok(Self {
            ...
            croncat_addr: cfg.contract_address.unwrap(),
        })
    }
