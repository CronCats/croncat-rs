...
use cosm_orc::orchestrator::{Coin, ChainResponse, ChainTxResponse};
use cosmrs::bip32;
...
impl Agent {
    pub async fn new(cfg: ChainConfig, contract_addr: String, key: bip32::XPrv, signer: Signer, querier: Querier) -> Result<Self, Report> {
    ...
    pub async fn register_agent(
        ...
        self.signer.execute_croncat(AgentExecuteMsg::RegisterAgent {
    ...
    pub async fn unregister_agent(&self) -> Result<ChainResponse, Report> {
        self.signer.execute_croncat(AgentExecuteMsg::UnregisterAgent { from_behind: None })
            .await
    }

    pub async fn update_agent(&self, payable_account_id: String) -> Result<ChainResponse, Report> {
        self.signer.execute_croncat(AgentExecuteMsg::UpdateAgent { payable_account_id })
            .await
    }

    pub async fn withdraw_reward(&self) -> Result<ChainResponse, Report> {
        self.signer.execute_croncat(ManagerExecuteMsg::AgentWithdraw(None))
            .await
    }
    ...
    pub async fn check_in_agent(&self) -> Result<ChainResponse, Report> {
        self.signer.execute_croncat(AgentExecuteMsg::CheckInAgent {}).await
    }
    ...
    pub async fn query_native_balance(&self, account: Option<String>) -> Result<Coin, Report> {
        let account_id: String = account.unwrap_or(self.account_id.clone()).clone().to_string();
        self.querier.rpc_client.query_balance(account_id.as_str()).await
    }

    pub async fn get_agent_status(&self, account_id: String) -> Result<AgentStatus, Report> {
        let agent_info: Option<AgentResponse> = self
            .querier.query_croncat(AgentQueryMsg::GetAgent { account_id })
            .await?;
                ...
                if block_counter.is_at_interval() {
                    ...
                    let agent = client
                        .execute(move |signer| {
                            ...
                            async move {
                                let agent = self.get_agent(account_id.as_str()).await?;
                                Ok(agent)
                            }
                        })
                        .await?;
                        ...
                        info!(
                            ...
                            client
                                .execute(|signer| async move {
                                    self.check_in_agent().await.map(|result| result.log)
                                })
                                .await?
                        );
                        let agent = client
                    ...
                    if let Some(threshold) = chain_config.threshold {
                        ...
                        let agent_balance = client
                            .execute(move |signer| async move {
                                let agent = self.query_native_balance(Some(account_id)).await?;
                                Ok(agent)
                            })
                            .await?;
                        ...
                        if agent_native_balance < threshold as u128 {
                            let agent = client
                                .execute(move |signer| async move {
                                    let agent = self.get_agent(account_str).await?;
                                    Ok(agent)
                                })
                                .await?;
                            ...
                            if !reward_balance.is_zero() {
                                ...
                                let result = client
                                    .execute(move |signer| async move {
                                        let agent = self.withdraw_reward().await?;
                                        Ok(agent)
                                    })
                                    .await?;
                                let log = result.log;
                                ...
                                let native_balance_after_withdraw = client
                                    .execute(move |signer| async move {
                                        let agent = self.query_native_balance(account_str).await?;
                                        Ok(agent)
                                    })
                                    .await?
                                    .amount;
